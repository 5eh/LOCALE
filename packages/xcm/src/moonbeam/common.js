...
import {
  ...
  ScheduleActionType,
  calculateTimeout,
  getTimeSlotSpanTimestamp,
  listenEvents,
  listenXcmpTaskEvents,
} from "../common/utils";
...
export const scheduleTask = async ({
  ...
  const moonbeamApi = await ApiPromise.create({
    provider: new WsProvider(moonbeamConfig.endpoint),
  });
  console.log("----------------------");
  console.log(moonbeamConfig.endpoint);
  console.log("----------------------");
  ...
  const oakAddress = keyring.encodeAddress(
    keyringPair.addressRaw,
    oakChainData.ss58Prefix
  );
  const moonbeamAddress = moonbeamKeyringPair.address;

  const proxyAccountId = oakAdapter.getDerivativeAccount(
    moonbeamKeyringPair.address,
    moonbeamChainData.paraId
  );
  const proxyAddressOnOak = keyring.encodeAddress(
    proxyAccountId,
    oakChainData.ss58Prefix
  );
  ...
  const { data: derivativeAccountBalanceOnOak } =
    await oakApi.query.system.account(proxyAddressOnOak);
  if (
    derivativeAccountBalanceOnOak.free.lt(MIN_BALANCE_IN_DERIVIATIVE_ACCOUNT)
  ) {
    const topUpExtrinsic = oakApi.tx.balances.transfer(
      proxyAddressOnOak,
      MIN_BALANCE_IN_DERIVIATIVE_ACCOUNT
    );
  ...
  const proxyAccountIdOnMoonbeam = moonbeamAdapter.getDerivativeAccount(
    proxyAccountId,
    oakChainData.paraId
  );
  const proxyTypeMoonbeam = "Any";
  const proxiesOnMoonbeam = _.first(
    (
      await moonbeamApi.query.proxy.proxies(
        u8aToHex(moonbeamKeyringPair.addressRaw)
      )
    ).toJSON()
  );

  const proxyMatchMoonbeam = _.find(
    proxiesOnMoonbeam,
    ({ delegate, proxyType }) =>
      _.toLower(delegate) === _.toLower(proxyAccountIdOnMoonbeam) &&
      proxyType === proxyTypeMoonbeam
  );
  if (proxyMatchMoonbeam) {
    console.log(
      `Proxy address ${proxyAccountIdOnMoonbeam} for paraId: ${moonbeamChainData.paraId} and proxyType: ${proxyTypeMoonbeam} already exists; skipping creation ...`
    );
  } else {
    console.log(
      `Add a proxy of ${moonbeamChainData.key} (paraId:${moonbeamChainData.paraId}) and proxyType: ${proxyTypeMoonbeam} on Turing ...\n Proxy address: ${proxyAccountIdOnMoonbeam}\n`
    );
    await sendExtrinsic(
      moonbeamApi,
      moonbeamApi.tx.proxy.addProxy(
        proxyAccountIdOnMoonbeam,
        proxyTypeMoonbeam,
        0
      ),
      moonbeamKeyringPair
  ...
  const { data: moonbaseProxyBalance } = await moonbeamApi.query.system.account(
    proxyAccountIdOnMoonbeam
  );
  ...
  if (moonbaseProxyBalance.free.lt(minBalanceOnMoonbaseProxy)) {
    const topUpExtrinsic = moonbeamApi.tx.balances.transfer(
      proxyAccountIdOnMoonbeam,
      minBalanceOnMoonbaseProxy
    );
  ...
  const sendExtrinsicPromise =
    Sdk().scheduleXcmpTimeTaskWithPayThroughRemoteDerivativeAccountFlow(
      {
        oakAdapter,
  ...
  const executionTime =
    scheduleActionType === ScheduleActionType.executeImmediately
      ? Math.floor(moment() / 1000) + 60
      : nextExecutionTime;
  const [, { taskId, messageHash }] = await waitPromises([
    sendExtrinsicPromise,
    listenXcmpTaskEvents(oakApi, executionTime),
  ]);
  ...
  const result = await listenEvents(
    ...
    "xcmpQueue",
    "Success",
    ({ messageHash: messageHashInEvent }) =>
      messageHashInEvent.toString() === messageHash.toString(),
    60000
  );
  if (_.isNull(result)) {
  ...
  const { events: xcmpQueueEvents, foundEventIndex: xcmpQueuefoundEventIndex } =
    result;
  const proxyExecutedEvent = _.findLast(
    xcmpQueueEvents,
    ({ event: { section, method } }) =>
      section === "ethereum" && method === "Executed",
    xcmpQueuefoundEventIndex + 1
  );
  console.log(
    "ethereum.Executed event: ",
    JSON.stringify(proxyExecutedEvent.event.data.toHuman())
  );

  if (scheduleActionType === ScheduleActionType.executeImmediately) return;

  console.log("\n5. Cancel the task ...");
  const cancelTaskExtrinsic = oakApi.tx.automationTime.cancelTaskWithScheduleAs(
    proxyAccountId,
    taskId
  );
  ...
  console.log(
    `\n6. Keep Listening events on ${parachainName} until ${moment(
      twoTimeSlotsTimestamp * 1000
    ).format(
      "YYYY-MM-DD HH:mm:ss"
    )}(${twoTimeSlotsTimestamp}) to verify that the task was successfully canceled ...`
  );

...
export const getHourlyTimestamp = (hour) =>
  moment().add(hour, "hour").startOf("hour").valueOf();
 ...
 * @returns promise
 */
export const waitPromises = (promises) =>
  new Promise((resolve, reject) => {
...
export const listenXcmpTaskEvents = async (oakApi, taskExecutionTime) => {
  ...
  const listenEventsResult = await listenEvents(
    ...
    "automationTime",
    "TaskExecuted",
    ({ taskId: taskIdInEvent }) =>
      Buffer.from(taskIdInEvent).toString() === taskId,
    timeout,
  );

  const { events, foundEventIndex: taskExecutedEventIndex } =
    listenEventsResult;
  const xcmpMessageSentEvent = _.findLast(
    events,
    ({ event: { section, method } }) =>
      section === "xcmpQueue" && method === "XcmpMessageSent",
